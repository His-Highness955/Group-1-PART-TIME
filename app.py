...
with tab1:
    ...
    if st.button(" Calculate Risk Profile"):
        if not patient_name:
            ...
            # Marriage is hidden from UI but provided as "Yes" to prevent KeyError in model
            input_dict = {
                'gender': gender, 'age': age, 'hypertension': hypertension, 
                'heart_disease': heart_disease, 'ever_married': "Yes",
            ...
            for col, le in assets['encoders'].items():
                if col in processed_df.columns:
                    ...
                    # Safety check: if a value (like 'Student') isn't in classes, use default
            ...
            # Save data (dropping ever_married to match the requested clean interface)
            save_patient_data(patient_name, input_df.drop(columns=['ever_married']), "Stroke", round(final_score, 2), risk_lvl)
